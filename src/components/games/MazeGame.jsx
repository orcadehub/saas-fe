...
const MazeGame = ({ level, themeColor = '#6366f1', onComplete }) => {
  ...
  const findPath = useCallback(() => {
    ...
    while (queue.length > 0) {
      ...
      for (const [dr, dc] of dirs) {
        ...
        if (
          newRow >= 0 &&
          newRow < mazeData.grid.length &&
          newCol >= 0 &&
          newCol < mazeData.grid[0].length &&
          mazeData.grid[newRow][newCol] === 0 &&
          !visited.has(key)
        ) {
          visited.add(key);
  ...
  const cellSize = Math.min(Math.floor(400 / mazeData.grid.length), Math.floor(400 / mazeData.grid[0].length));
      ...
      >
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 1, mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            <Box sx={{ px: 0.5, py: 0.2, bgcolor: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '4px', borderBottomWidth: '2px' }}>
                      ...
                      sx={{
                        ...
                        bgcolor: isEnd
                          ...
                          : isStart
                            ...
                            : isPathHint
                              ? '#fde047'
                              : hitWallPos && hitWallPos[0] === rowIdx && hitWallPos[1] === colIdx
                                ? 'rgba(239, 68, 68, 0.3)'
                                : '#f8fafc',
                        border: '1px solid',
                        borderColor: '#e2e8f0',
