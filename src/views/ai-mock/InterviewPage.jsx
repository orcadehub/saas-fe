        ...
        if (wordIndex < words.length) {
          const highlighted = words
            .map((word, idx) => {
              ...
              if (idx === wordIndex)
                return `<span style="color: #1976d2; font-weight: 700; background: rgba(25, 118, 210, 0.1); padding: 2px 4px; border-radius: 4px;">${word}</span>`;
            ...
            <Box sx={{ position: 'absolute', top: 20, left: 20 }}>
              <Alert severity="error" sx={{ bgcolor: 'rgba(211, 47, 47, 0.9)' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Videocam />
                  <Typography variant="body2">{isRecording ? 'REC' : 'Not Recording'}</Typography>
                </Box>
              </Alert>
            </Box>
