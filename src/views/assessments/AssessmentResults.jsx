...
import { ArrowBack, ExpandMore, CheckCircle, Cancel, Code, Timer } from '@mui/icons-material';
    ...
    <Box sx={{ borderRadius: '16px', overflow: 'hidden', border: '1px solid #0f172a', bgcolor: '#0f172a', mt: 1.5, position: 'relative' }}>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Typography
            variant="caption"
            sx={{ color: '#94a3b8', fontWeight: 800, textTransform: 'uppercase', fontSize: '0.65rem', letterSpacing: '0.05em' }}
          >
            {title || 'Source Code'}
          </Typography>
        </Box>
      ...
      <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 3, bgcolor: '#f8fafc' }}>
        ...
        >
          ...
          <Typography variant="body1" sx={{ color: '#64748b', mb: 5, fontSize: '1.1rem', fontWeight: 500 }}>
            The assessment results are currently locked and will be automatically released once the timer expires.
          </Typography>
    ...
    <Box sx={{ bgcolor: '#f8fafc', minHeight: '100vh', pb: 8 }}>
        ...
        <Box sx={{ px: { xs: 3, md: 6 }, maxWidth: 1400, mx: 'auto' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 4 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5, cursor: 'pointer' }} onClick={() => navigate('/assessments')}>
              ...
              <Box>
                <Typography variant="h2" sx={{ fontWeight: 900, color: '#0f172a', fontSize: '2.25rem' }}>
                  Assessment Analysis
                </Typography>
      ...
      <Box sx={{ px: { xs: 3, md: 6 }, maxWidth: 1400, mx: 'auto' }}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            ...
            >
              <Box sx={{ p: 2.25 }}>
                ...
                <Typography sx={{ mb: 1.25, fontSize: '1rem', fontWeight: 500, color: 'secondary.200' }}>Time Consumed</Typography>
              </Box>
            </MainCard>
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            ...
            >
              <Box sx={{ p: 2.25 }}>
                ...
                <Typography sx={{ mb: 1.25, fontSize: '1rem', fontWeight: 500, color: 'error.light' }}>Security Violations</Typography>
              </Box>
            </MainCard>
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            ...
            >
              <Box sx={{ p: 2.25 }}>
                ...
                <Typography sx={{ mb: 1.25, fontSize: '1rem', fontWeight: 500, color: 'rgba(255,255,255,0.7)' }}>Task Progress</Typography>
              </Box>
            </MainCard>
          </Grid>
              ...
              <Grid container spacing={4}>
                {qCounts.quiz > 0 && (
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    <LinearProgress
                      variant="determinate"
                      value={attempt?.quizPercentage || 0}
                      sx={{ height: 6, borderRadius: 3, bgcolor: '#334155', '& .MuiLinearProgress-bar': { bgcolor: '#6366f1' } }}
                    />
                  </Grid>
                )}
                  ...
                  <Grid item xs={12} sm={6} md={3}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      ...
                      <Typography sx={{ color: '#10b981', fontWeight: 700 }}>
                        {(attempt?.programmingPercentage || 0).toFixed(0)}%
                      </Typography>
                    </Box>
                    <LinearProgress
                      variant="determinate"
                      value={attempt?.programmingPercentage || 0}
                      sx={{ height: 6, borderRadius: 3, bgcolor: '#334155', '& .MuiLinearProgress-bar': { bgcolor: '#10b981' } }}
                    />
                  </Grid>
                )}
                {qCounts.mongodb > 0 && (
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    <LinearProgress
                      variant="determinate"
                      value={attempt?.mongodbPercentage || 0}
                      sx={{ height: 6, borderRadius: 3, bgcolor: '#334155', '& .MuiLinearProgress-bar': { bgcolor: '#ec4899' } }}
                    />
                  </Grid>
                )}
                {qCounts.frontend > 0 && (
                  <Grid item xs={12} sm={6} md={3}>
                    ...
                    <LinearProgress
                      variant="determinate"
                      value={attempt?.frontendPercentage || 0}
                      sx={{ height: 6, borderRadius: 3, bgcolor: '#334155', '& .MuiLinearProgress-bar': { bgcolor: '#0ea5e9' } }}
                    />
                  </Grid>
                )}
                {(qCounts.sql || 0) > 0 && (
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {tabValue === 0 &&
            assessment?.quizQuestions?.map((q, i) => {
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2, mt: 3 }}>
                    {q.options?.map((opt, idx) => {
                      ...
                      return (
                        <Box
                          key={idx}
                          sx={{ p: 2, borderRadius: '16px', border, bgcolor: bg, display: 'flex', gap: 2, alignItems: 'center' }}
                        >
                          <Box
                            sx={{
                              width: 24,
                              height: 24,
                              borderRadius: '6px',
                              bgcolor: isCorrect ? '#10b981' : selected ? '#ef4444' : '#f1f5f9',
                              color: isCorrect || selected ? 'white' : '#64748b',
                              display: 'flex',
                              alignItems: 'center',
                              justifyContent: 'center',
                              fontWeight: 900
            ...
            assessment?.questions?.map((q, i) => {
              ...
              return (
                <Card key={q._id} sx={{ borderRadius: '32px', overflow: 'hidden', border: '1px solid #f1f5f9' }}>
                  <Box
                    sx={{
                      bgcolor: score >= 70 ? '#dcfce7' : score > 0 ? '#fef3c7' : '#fee2e2',
                      p: 3,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'space-between',
                  ...
                  >
                    ...
                    <Typography
                      sx={{ fontWeight: 900, color: score >= 70 ? '#16a34a' : score > 0 ? '#d97706' : '#dc2626', fontSize: '1.5rem' }}
                    >
                      {score.toFixed(0)}%
                    </Typography>
                  </Box>
                  <Box sx={{ p: 4 }}>
                    <Typography
                      sx={{ mb: 3, fontWeight: 600, color: '#475569', lineHeight: 1.7 }}
                        ...
                        {Object.keys(codeObj).map((lang) => (
                          ...
                          >
                            <AccordionSummary expandIcon={<ExpandMore />}>
                              <Typography sx={{ fontWeight: 800, color: '#6366f1', textTransform: 'uppercase', fontSize: '0.75rem' }}>
                                {lang} source
                              </Typography>
                            </AccordionSummary>
          ...
          {tabValue === 2 &&
            assessment?.frontendQuestions?.map((q, i) => {
          ...
          {tabValue === 3 &&
            assessment?.mongodbPlaygroundQuestions?.map((q, i) => {
                ...
                <Card key={q._id} sx={{ borderRadius: '32px', overflow: 'hidden', border: '1px solid #f1f5f9' }}>
                  <Box sx={{ bgcolor: isCorrect ? '#dcfce7' : '#fee2e2', p: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Box>
                      ...
                      <Typography variant="caption" sx={{ color: '#475569', fontWeight: 800 }}>
                        {q.title || q.collectionName || 'Query'}
                      </Typography>
                    </Box>
                  </Box>
                  <Box sx={{ p: 4 }}>
                    <Typography
                      sx={{ mb: 3, fontWeight: 600, color: '#475569', lineHeight: 1.7 }}
                      dangerouslySetInnerHTML={{ __html: q.description || q.problemStatement || '' }}
                    />
                    <Typography variant="subtitle2" sx={{ fontWeight: 800, mb: 1, color: '#64748b' }}>
                      Submitted Query
                    </Typography>
          ...
          {tabValue === 4 &&
            assessment?.sqlPlaygroundQuestions?.map((q, i) => {
