...
export default function SQLPractice() {
  ...
  const fetchAssessment = async () => {
    try {
      ...
      setQuestions(questionsData.sqlPlaygroundQuestions || []);
    } catch (error) {
    ...
    <Box ref={containerRef} sx={{ display: 'flex', flexDirection: 'column', height: '100vh', bgcolor: '#fbfcfe' }}>
      {!isFullscreen && (
        <Box sx={{ 
          ...
          <Typography variant="h5" sx={{ fontWeight: 800, flexGrow: 1, color: '#1e293b' }}>
            {assessment.title} <Typography component="span" sx={{ fontWeight: 600, color: '#64748b', fontSize: '0.9rem', ml: 1 }}>• SQL Practice</Typography>
          </Typography>
          <Chip 
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: `${leftWidth}% 4px 1fr`, flexGrow: 1, height: '100%', overflow: 'hidden' }}>
        ...
        <Box sx={{ bgcolor: '#fff', height: '100%', minWidth: 0 }}>
          <SQLPlaygroundEditor assessment={assessment} question={currentQuestion} attemptId={null} onTestComplete={() => {}} />
        </Box>
      </Box>
    </Box>
  );
