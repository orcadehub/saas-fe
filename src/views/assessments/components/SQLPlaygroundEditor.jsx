    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (attemptId && question._id) {
          try {
            await apiService.saveSQLQuery(token, attemptId, question._id, query, response.data.result, expectedOutput, isCorrect);
          } catch (err) {
        ...
        {currentTab === 1 && (
          <Box sx={{ p: 4 }}>
            ...
            <Typography variant="body1" sx={{ color: '#64748b', mb: 3 }}>
              Currently interacting with table:{' '}
              <Typography component="span" sx={{ fontWeight: 800, color: '#6366f1' }}>
                {question?.tableName}
              </Typography>
            </Typography>
                ...
                <CardContent sx={{ p: 3 }}>
                  <Box sx={{ overflow: 'auto', maxHeight: '500px' }}>
                    {expectedOutput.length > 0 ? (
                      <table
                        style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'JetBrains Mono', monospace", fontSize: '14px' }}
                      >
                        <thead>
                          <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                            {Object.keys(expectedOutput[0]).map((key) => (
                              <th key={key} style={{ padding: '12px', textAlign: 'left', fontWeight: 800, color: '#475569' }}>
                                {key}
                              </th>
                            ))}
                          </tr>
                          ...
                          {expectedOutput.map((row, i) => (
                            <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                              {Object.values(row).map((val, j) => (
                                <td key={j} style={{ padding: '12px', color: '#1e293b' }}>
                                  {String(val)}
                                </td>
                              ))}
                            </tr>
          ...
          <Box sx={{ p: 4 }}>
            ...
            {error && (
              <Card sx={{ borderRadius: '16px', border: '1px solid rgba(239, 68, 68, 0.2)', bgcolor: 'rgba(239, 68, 68, 0.02)', mb: 3 }}>
                <CardContent sx={{ p: 3 }}>
                  <Typography variant="subtitle2" sx={{ color: '#ef4444', fontWeight: 800, textTransform: 'uppercase', mb: 1 }}>
                    SQL Error
                  </Typography>
            ...
            {!result && !error && (
              <Box sx={{ py: 12, textAlign: 'center' }}>
                <Typography sx={{ color: '#94a3b8', fontWeight: 600 }}>
                  Execute your SQL query in the Editor tab to see results here.
                </Typography>
              </Box>
            )}
          </Box>
