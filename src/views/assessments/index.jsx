...
export default function Assessments() {
      ...
      return (
        ...
        (!attempt ||
          (attempt.attemptStatus !== 'COMPLETED' &&
            attempt.attemptStatus !== 'TAB_SWITCH_VIOLATION' &&
            attempt.attemptStatus !== 'TERMINATED') ||
          attempt.attemptStatus === 'RESUMED' ||
  ...
  const expiredAssessments = assessments
    .filter((assessment) => {
      const startTime = new Date(assessment.startTime).getTime();
      const endTime = startTime + assessment.duration * 60 * 1000;
      const attempt = assessmentAttempts[assessment._id];

      return now >= endTime && (!attempt || (attempt.attemptStatus !== 'COMPLETED' && attempt.attemptStatus !== 'TAB_SWITCH_VIOLATION'));
    })
    .sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime());
              ...
              >
                <Stack spacing={1}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography
                      variant="caption"
                      sx={{ fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', fontSize: '0.65rem' }}
                    >
                      Start Schedule
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Typography
                      variant="body2"
                      sx={{ fontWeight: 700, color: '#6366f1', bgcolor: 'rgba(99, 102, 241, 0.08)', px: 1, py: 0.2, borderRadius: '6px' }}
                    >
            ...
            <>
              {tabValue === 0 &&
                (availableAssessments.length > 0 ? (
                  renderAssessmentCards(availableAssessments, 'available')
                ) : (
                  <Box sx={{ ...whiteGlassSx, textAlign: 'center', py: 8 }}>
                    <Typography variant="h4" sx={{ fontWeight: 800, color: '#1e293b' }}>
                      No live assessments found
                    </Typography>
                    <Typography variant="body1" sx={{ color: '#64748b', mt: 1 }}>
                      Check back later for scheduled tasks
                    </Typography>
                  </Box>
                ))}
              {tabValue === 1 &&
                (completedAssessments.length > 0 ? (
                  renderAssessmentCards(completedAssessments, 'completed')
                ) : (
                  <Box sx={{ ...whiteGlassSx, textAlign: 'center', py: 8 }}>
                    <Typography variant="h4" sx={{ fontWeight: 800, color: '#1e293b' }}>
                      No completed exams
                    ...
                    <Typography variant="body1" sx={{ color: '#64748b', mt: 1 }}>
                      Your submitted assessments will appear here
                    </Typography>
                  </Box>
                ))}
              {tabValue === 2 &&
                (expiredAssessments.length > 0 ? (
                  renderAssessmentCards(expiredAssessments, 'expired')
                ) : (
                  <Box sx={{ ...whiteGlassSx, textAlign: 'center', py: 8 }}>
                    <Typography variant="h4" sx={{ fontWeight: 800, color: '#1e293b' }}>
                      Empty history
