  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
      {text.split('\n').map((line, i) => {
        if (line.startsWith('## ')) {
          return (
            <Typography key={i} sx={{ fontWeight: 800, fontSize: '1rem', color: '#1e293b', mt: 1 }}>
              {line.slice(3)}
            </Typography>
          );
        }
        if (line.startsWith('**') && line.endsWith('**')) {
          return (
            <Typography key={i} sx={{ fontWeight: 700, color: '#1e293b', fontSize: '0.875rem' }}>
              {line.slice(2, -2)}
            </Typography>
          );
        }
        if (line.startsWith('- ')) {
          ...
          return (
            <Box key={i} sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
              ...
              <Typography sx={{ fontSize: '0.85rem', color: '#475569', lineHeight: 1.6 }}>{renderInline(content)}</Typography>
            </Box>
          );
        }
        if (line.trim() === '') return <Box key={i} sx={{ height: 4 }} />;
        return (
          <Typography key={i} sx={{ fontSize: '0.85rem', color: '#475569', lineHeight: 1.6 }}>
            {renderInline(line)}
          </Typography>
        );
      })}
...
function renderInline(text) {
  ...
  return parts.map((p, i) => {
    ...
    if (p.startsWith('`') && p.endsWith('`'))
      return (
        <Box
          key={i}
          component="span"
          sx={{ fontFamily: 'monospace', bgcolor: '#f1f5f9', px: 0.6, py: 0.1, borderRadius: '4px', fontSize: '0.82rem', color: '#6366f1' }}
        >
          {p.slice(1, -1)}
        </Box>
      );
    if (p.startsWith('**') && p.endsWith('**'))
      return (
        <Box key={i} component="span" sx={{ fontWeight: 700, color: '#1e293b' }}>
          {p.slice(2, -2)}
        </Box>
      );
    return p;
...
const DEFAULT_CELLS = [{ id: 1, code: '', output: null, running: false, executionCount: null }];
...
export default function GenAIPlayground() {
  ...
  const [solvedIds, setSolvedIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('genai_solved') || '[]');
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    if (geminiKey) localStorage.setItem('genai_gemini_key', geminiKey);
  }, [geminiKey]);
  useEffect(() => {
    if (hfKey) localStorage.setItem('genai_hf_key', hfKey);
  }, [hfKey]);
  ...
  const runCell = async (cellId) => {
    ...
    try {
      ...
      setCells((prev) =>
        prev.map((c) =>
          c.id === cellId
            ? {
                ...c,
                output: { stdout: data.stdout || '', stderr: data.stderr || '', success: data.success },
      ...
      setCells((prev) =>
        prev.map((c) => (c.id === cellId ? { ...c, output: { stdout: '', stderr: err.message, success: false }, running: false } : c))
      );
    }
  };
  ...
  return (
    <Box
      ref={containerRef}
      sx={{ height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: '#fbfcfe', position: 'relative', overflow: 'hidden' }}
    >
      <LightBackground />
                ...
                <Box key={cell.id}>
                  <Paper
                    ...
                    sx={{
                      borderRadius: '20px',
                      border: '1px solid',
                      borderColor: cell.running ? '#6366f1' : '#f1f5f9',
                      bgcolor: '#fff',
                  ...
                  >
                    ...
                    >
                      ...
                      <Box sx={{ flex: 1 }} />
                      <IconButton size="small" onClick={() => moveCell(cell.id, 'up')} disabled={idx === 0}>
                        <KeyboardArrowUp sx={{ fontSize: 16 }} />
                      </IconButton>
                      <IconButton size="small" onClick={() => moveCell(cell.id, 'down')} disabled={idx === cells.length - 1}>
                        <KeyboardArrowDown sx={{ fontSize: 16 }} />
                      </IconButton>
                      <IconButton size="small" onClick={() => deleteCell(cell.id)} disabled={cells.length === 1}>
                      ...
                      <Button
                        ...
                        sx={{ ml: 1, bgcolor: '#6366f1', color: '#fff', borderRadius: '8px', fontSize: '0.7rem' }}
                      >
                        Run
                      </Button>
                    </Box>
                    ...
                    {cell.output && (
                      <Box sx={{ p: 2, bgcolor: cell.output.success ? '#fafafa' : '#fff8f8', borderTop: '1px solid #f1f5f9' }}>
                        {cell.output.stdout && (
                          <Box sx={{ p: 1.5, bgcolor: '#f8fafc', borderRadius: '8px', fontFamily: 'monospace', fontSize: '0.85rem' }}>
                            {cell.output.stdout}
                          </Box>
                        )}
                        {cell.output.stderr && (
            ...
            <>
              ...
              {selectedQuestion ? (
                <Box sx={{ p: 4, maxWidth: 800, mx: 'auto' }}>
                  <Typography variant="h4" sx={{ fontWeight: 900, mb: 1.5, color: '#1e293b' }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mb: 4 }}>
                    <Chip
                      ...
                      size="small"
                      sx={{ fontWeight: 800, bgcolor: diffBg[selectedQuestion.difficulty], color: diffColor[selectedQuestion.difficulty] }}
                    />
                    {selectedQuestion.requiredLibraries?.map((l) => (
                      <Chip key={l} label={l} size="small" sx={{ fontWeight: 700 }} />
                    ))}
                  </Box>
                  <Typography
                    sx={{
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box
                sx={{
                  p: 2,
                  borderRadius: '14px',
                  bgcolor: evalResult.allPassed ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)',
                  display: 'flex',
                  alignItems: 'center',
                  gap: 2
                }}
              ...
              {evalResult.parsedTests.length > 0 && (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  <Typography
                    sx={{ fontSize: '0.75rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em' }}
                  >
                    Test Cases
                  </Typography>
                  {evalResult.parsedTests.map((t, i) => (
                    <Box
                      ...
                      sx={{
                        p: 1.5,
                        borderRadius: '12px',
                        bgcolor: t.passed ? 'rgba(16,185,129,0.04)' : 'rgba(239,68,68,0.04)',
                        border: '1px solid',
                        borderColor: t.passed ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)',
                        display: 'flex',
                        alignItems: 'center',
                        gap: 2
                      }}
                    >
                      ...
                      <Typography sx={{ fontWeight: 700, fontSize: '0.85rem', flex: 1, color: '#1e293b', fontFamily: 'monospace' }}>
                        {t.name}
                      </Typography>
                      <Chip
                        ...
                        sx={{
                          ...
                          fontSize: '0.65rem',
                          fontWeight: 800,
                          bgcolor: t.passed ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)',
              ...
              {evalResult.parsedTests.length === 0 && (
                <Typography sx={{ color: '#ef4444', fontStyle: 'italic', textAlign: 'center', py: 2 }}>
                  No test output received. Make sure your code defines the required variables.
                </Typography>
              )}
            </Box>
          ) : (
