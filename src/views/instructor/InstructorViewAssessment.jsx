  ...
  const executeCode = async () => {
    ...
    for (const testCase of selectedQuestionForRun.testCases) {
      try {
        ...
        const isCorrect =
          result.run?.code === 0 && result.run?.stdout?.trim().replace(/\s+/g, ' ') === testCase.output.trim().replace(/\s+/g, ' ');
        results.push({
          ...testCase,
          actualOutput: result.run?.stdout?.trim() || result.run?.stderr || 'No output',
          isCorrect,
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h3" sx={{ fontWeight: 700 }}>
                {assessment.maxTabSwitches === -1 ? '∞' : assessment.maxTabSwitches}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
                  ...
                  <CardContent>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        <Button size="small" variant="contained" color="success" onClick={() => handleRunCode(question)}>
                          Run
                        </Button>
                        <Button
                          size="small"
        ...
        <DialogContent>
                        ...
                        <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <Button
                              ...
                              disabled={addToAssessmentLoading === question._id}
                              onClick={() =>
                                assessment?.questions?.some((q) => q._id === question._id)
          ...
          {tabValue === 1 && (
            <Box sx={{ height: 'auto', display: 'flex', flexDirection: 'column' }}>
              ...
              <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, overflow: 'auto', mb: 2 }}>
                {(() => {
                  const filtered = availableQuizQuestions.filter(
                    (q) =>
                      q.title?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                      q.description?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                      q.tags?.some((tag) => tag.toLowerCase().includes(searchQuery.toLowerCase()))
                        ...
                        <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <Button
                              ...
                              disabled={addToAssessmentLoading === question._id}
                              onClick={() =>
                                assessment?.quizQuestions?.some((q) => q._id === question._id)
                                  ? removeQuizQuestionFromAssessment(question._id)
          ...
          <Grid container spacing={2} sx={{ mt: 1, height: '70vh' }}>
            <Grid item xs={6} sx={{ display: 'flex', flexDirection: 'column', height: '100%', width: '48%' }}>
              ...
              <TextField
                ...
                sx={{ fontFamily: 'monospace', mb: 2, flex: 1 }}
              />
              <Button variant="contained" fullWidth onClick={executeCode} disabled={runningTests || !code.trim()}>
                ...
                {testResults.length > 0 ? (
                  testResults.map((result, idx) => (
                    <Card
                      ...
                      sx={{
                        mb: 2,
                        border: `2px solid ${result.isCorrect ? 'green' : 'red'}`,
                        bgcolor: result.isCorrect ? '#e8f5e9' : '#ffebee'
                      }}
                    >
                      <CardContent>
                        <Typography variant="subtitle2">
                          Test Case {idx + 1} - {result.isCorrect ? 'PASS' : 'FAIL'}
                        </Typography>
