...
export default function Leaderboard() {
  ...
  return (
    ...
    >
        ...
        >
          ...
          <Stack direction="row" spacing={2} sx={{ width: { xs: '100%', md: 'auto' } }}>
            <FormControl sx={{ minWidth: 150, ...lightInputSx }}>
              <Select value={timeframe} onChange={(e) => setTimeframe(e.target.value)} displayEmpty>
            ...
            <Table sx={{ minWidth: 800 }}>
              <TableHead sx={{ bgcolor: '#f8fafc' }}>
                <TableRow>
                  ...
                  <TableCell align="center" sx={{ fontWeight: 800, color: '#475569', py: 2 }}>
                    LEETCODE
                  </TableCell>
                  <TableCell align="center" sx={{ fontWeight: 800, color: '#475569', py: 2 }}>
                    HACKERRANK
              ...
              <TableBody>
                {leaderboardData.slice((page - 1) * 50, page * 50).map((student) => {
                  ...
                  return (
                    <TableRow
                      key={student._id}
                      sx={{ bgcolor: isMe ? 'rgba(99, 102, 241, 0.03)' : 'inherit', '&:hover': { bgcolor: '#fbfcfe' } }}
                    >
                      <TableCell sx={{ py: 1.5 }}>
                        <Box
                          sx={{
                      ...
                      <TableCell sx={{ py: 1.5 }}>
                        <Stack direction="row" spacing={2} alignItems="center">
                          <Avatar sx={{ width: 34, height: 34, bgcolor: isMe ? '#6366f1' : '#e2e8f0', fontSize: '0.8rem' }}>
                      ...
                      <TableCell align="center" sx={{ py: 1.5 }}>
                        <Typography sx={{ fontWeight: 700, color: '#f59e0b', fontSize: '0.875rem' }}>{student.leetcode || 0}</Typography>
                      </TableCell>
                      <TableCell align="center" sx={{ py: 1.5 }}>
                        <Typography sx={{ fontWeight: 700, color: '#10b981', fontSize: '0.875rem' }}>{student.hackerrank || 0}</Typography>
                      </TableCell>
                      <TableCell align="center" sx={{ py: 1.5 }}>
                        <Typography sx={{ fontWeight: 700, color: '#3b82f6', fontSize: '0.875rem' }}>{student.codeforces || 0}</Typography>
                      </TableCell>
                      <TableCell align="center" sx={{ py: 1.5 }}>
                        <Typography sx={{ fontWeight: 700, color: '#6366f1', fontSize: '0.875rem' }}>{student.appSolved || 0}</Typography>
                      </TableCell>
                      <TableCell align="center" sx={{ py: 1.5 }}>
                        <Box
                          sx={{
                            ...
                            py: 0.5,
                            borderRadius: '8px',
                            bgcolor: student.rank <= 3 ? '#6366f1' : '#f1f5f9',
                            color: student.rank <= 3 ? '#fff' : '#475569',
                            fontWeight: 800,
      ...
      {/* Sync Dialog */}
      <Dialog open={openDialog} onClose={() => !connecting && setOpenDialog(false)} maxWidth="xs" fullWidth sx={lightDialogSx}>
      ...
      <Dialog open={statsDialog} onClose={() => setStatsDialog(false)} maxWidth="sm" fullWidth sx={lightDialogSx}>
            ...
            <Stack spacing={3}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
                {[
                  {
                    label: 'LEETCODE',
                    value: selectedStudent.leetcode || 0,
                    handle: selectedStudent.leetcodeUsername || 'Not Linked',
                    color: '#f59e0b',
                    bg: '#fffbeb',
                    border: '#fde68a'
                  },
                  {
                    label: 'HACKERRANK',
                    value: selectedStudent.hackerrank || 0,
                    handle: selectedStudent.hackerrankUsername || 'Not Linked',
                    color: '#10b981',
                    bg: '#f0fdf4',
                    border: '#bbf7d0'
                  },
                  {
                    label: 'CODEFORCES',
                    value: selectedStudent.codeforces || 0,
                    handle: selectedStudent.codeforcesUsername || 'Not Linked',
                    color: '#3b82f6',
                    bg: '#eff6ff',
                    border: '#bfdbfe'
                  },
                  {
                    label: 'ORCA SOLVED',
                    value: selectedStudent.appSolved || 0,
                    handle: selectedStudent.email?.split('@')[0],
                    color: '#6366f1',
                    bg: '#f5f3ff',
                    border: '#ddd6fe'
                  },
                  {
                    label: 'TOTAL SCORE',
                    value: selectedStudent.totalSolved || 0,
                    color: '#ef4444',
                    bg: '#fef2f2',
                    border: '#fecaca',
                    span: 2
                  ...
                  >
                    <Typography
                      variant="caption"
                      sx={{ color: stat.color, fontWeight: 800, display: 'block', mb: 0.5, letterSpacing: '0.05em' }}
                    >
                      {stat.label}
                    </Typography>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                      <Typography variant="h3" sx={{ color: stat.color, fontWeight: 900 }}>
                        {stat.value}
                      </Typography>
                      {stat.handle && (
              ...
              >
                ...
                <Typography variant="h1" sx={{ fontWeight: 900, fontSize: '2.5rem' }}>
        ...
        <DialogActions sx={{ p: 3, justifyContent: 'center' }}>
          <Button fullWidth onClick={() => setStatsDialog(false)} sx={{ fontWeight: 700, color: '#64748b' }}>
            Dismiss
          </Button>
        </DialogActions>
      </Dialog>
