...
export default function DSAPractice() {
  ...
  useEffect(() => {
    // Map JSON data to topic objects
    const mappedTopics = dsaData.map(t => ({
      topic: t.topicName,
  ...
  return (
    <Box sx={{ p: { xs: 2.5, sm: 3, md: 4 } }}>
      <Breadcrumbs separator={<IconChevronRight size={16} />} sx={{ mb: 4 }}>
        ...
        <Typography sx={{ fontWeight: 800, color: '#1e293b' }}>DSA Practice</Typography>
      </Breadcrumbs>
      ...
      <Box sx={{ mb: 6 }}>
        ...
        <Typography variant="body1" sx={{ color: '#64748b', fontSize: '1.1rem', fontWeight: 500 }}>
          Master data structures and algorithms with curated topics and questions.
        </Typography>
      </Box>
        ...
        {topics.map((topic, idx) => {
          ...
          return (
            <MotionCard
              ...
              onClick={() => navigate(`/practice/dsa/${encodeURIComponent(topic.topic)}`)}
            >
              <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', p: { xs: 2.5, sm: 3 } }}>
                ...
                >
                  Explore {topic.topic} problems and track your progress.
                </Typography>
              </CardContent>
            </MotionCard>
          );
