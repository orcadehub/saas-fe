    ...
    <MainCard>
      <Breadcrumbs separator={<IconChevronRight size={16} />} sx={{ mb: 3 }}>
        ...
        <Link
          ...
          onClick={(e) => {
            e.preventDefault();
            navigate(`/practice/gamified/${topicId}`);
          }}
          sx={{ cursor: 'pointer' }}
        >
          Topic
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          {timeLeft !== null && (
            <Chip icon={<IconClock size={16} />} label={formatTime(timeLeft)} color={timeLeft < 30 ? 'error' : 'primary'} />
          )}
          <Chip icon={<IconStar size={16} />} label={`Score: ${score}`} color="secondary" />
        </Box>
      </Box>
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
          <Typography variant="body2">
            Level {currentLevel + 1} of {question.levels.length}
          </Typography>
                ...
                <FormControlLabel
                  ...
                  label={
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      {option.text}
                      {option.image && <img src={option.image} alt="Option" style={{ maxHeight: '50px', borderRadius: '4px' }} />}
                    </Box>
                  }
                  sx={{
      ...
      <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
        {!showResult ? (
          <Button variant="contained" onClick={handleSubmit} disabled={!selectedAnswer}>
            Submit Answer
          </Button>
        ) : (
