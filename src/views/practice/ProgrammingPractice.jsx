  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.DEV ? 'http://localhost:4000/api' : 'https://backend.orcode.in/api'}/practice-submissions/stats`,
        {
          headers: {
        ...
        {!loading && (
          ...
          >
            ...
            <LinearProgress
              ...
              sx={{ height: 12, borderRadius: 6, bgcolor: '#f1f5f9', '& .MuiLinearProgress-bar': { borderRadius: 5, bgcolor: '#6366f1' } }}
            />
            <Typography
              sx={{
                mt: 1.5,
            ...
            return (
              ...
              >
                ...
                <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', p: { xs: 3, sm: 3.5 } }}>
                  ...
                  <Typography
                    variant="h3"
                    sx={{ fontWeight: 800, color: '#1e293b', mb: 1.5, fontSize: '1.25rem', textTransform: 'capitalize' }}
                  >
                    {topic.topic}
                  </Typography>
                  ...
                  <Box sx={{ mt: 'auto' }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                      <Typography sx={{ fontSize: '0.7rem', fontWeight: 900, color: '#94a3b8', textTransform: 'uppercase' }}>
                        Topic Progress
                      </Typography>
                      <Typography sx={{ fontSize: '0.75rem', fontWeight: 900, color: isCompleted ? '#34d399' : '#64748b' }}>
                        {totalInTopic > 0 ? Math.round((solvedInTopic / totalInTopic) * 100) : 0}%
                      </Typography>
                    </Stack>
                    <LinearProgress
                      variant="determinate"
                      value={totalInTopic > 0 ? (solvedInTopic / totalInTopic) * 100 : 0}
                      sx={{
                        height: 6,
                        borderRadius: 3,
                        bgcolor: '#f1f5f9',
                        '& .MuiLinearProgress-bar': { borderRadius: 3, bgcolor: isCompleted ? '#34d399' : theme.color }
                      }}
                    />
                  </Box>
                </CardContent>
