  ...
  const fetchCompletedStatus = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.DEV ? 'http://localhost:4000/api' : 'https://backend.orcode.in/api'}/practice-submissions/completed`,
        {
          headers: {
              ...
              >
                ...
                <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', p: { xs: 2.5, sm: 3 } }}>
                  ...
                  <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mt: 2.5 }}>
                    ...
                    {isCompleted && (
                      <Typography
                        sx={{ color: '#059669', fontWeight: 900, fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: 0.5 }}
                      >
