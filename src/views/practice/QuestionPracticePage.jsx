...
export default function QuestionPracticePage() {
    ...
    for (let i = 0; i < allRunTC.length; i++) {
      ...
      try {
        ...
        setTestCaseResults((prev) => ({
          ...prev,
          [i]: { loading: false, output: userOut, error: result.stderr || (result.status.id !== 3 ? 'Execution Failed' : null), passed }
        }));
      } catch (err) {
        setTestCaseResults((prev) => ({ ...prev, [i]: { loading: false, error: err.message, passed: false } }));
      }
    }
    setIsRunning(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    const currentResults = allTC.map((tc, i) => ({
      index: i + 1,
      status: 'Running',
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/practice-submissions/submit`, {
        ...
        body: JSON.stringify({
          ...
          code,
          language,
          status: failCount === 0 ? 'accepted' : 'wrong_answer',
          passedTests: allTC.length - failCount,
          totalTests: allTC.length,
          executionTime: totalCpuTime,
  ...
  const startResizing = useCallback((e) => {
    const mm = (v) => {
      const nw = (v.clientX / window.innerWidth) * 100;
      if (nw > 20 && nw < 80) setLeftWidth(nw);
    };
    const stop = () => {
  ...
  const startVerticalResizing = useCallback((e) => {
    const mm = (v) => {
      if (!editorSplitRef.current) return;
      const rect = editorSplitRef.current.getBoundingClientRect();
      const nh = ((v.clientY - rect.top) / rect.height) * 100;
      if (nh > 20 && nh < 90) setCompilerSplit(nh);
    };
    const stop = () => {
      window.removeEventListener('mousemove', mm);
      window.removeEventListener('mouseup', stop);
    ...
    return (
      <Box sx={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: t.bg }}>
        <Stack spacing={3} alignItems="center">
          <CircularProgress thickness={5} size={60} sx={{ color: t.accent }} />
          <Typography sx={{ color: t.textSecondary, fontWeight: 700, letterSpacing: '0.1em' }} variant="overline">
            ORCA INITIALIZING...
          </Typography>
        </Stack>
      </Box>
    ...
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: t.bg, overflow: 'hidden', color: t.text }}>
      ...
      >
        <Stack direction="row" spacing={2} alignItems="center">
          <IconButton onClick={() => navigate(-1)} size="small" sx={{ color: t.textMuted }}>
            <KeyboardBackspace />
          </IconButton>
          <Breadcrumbs separator={<Typography sx={{ color: t.textMuted, fontSize: '0.7rem' }}>/</Typography>}>
            <Link
              component="button"
              variant="body2"
              onClick={() => navigate('/practice/programming')}
              sx={{ color: t.textMuted, textDecoration: 'none', fontWeight: 600, fontSize: '0.75rem' }}
            >
              Practice
            </Link>
            <Typography variant="body2" sx={{ color: t.text, fontWeight: 800, fontSize: '0.8rem' }}>
              {question.title}
            </Typography>
          </Breadcrumbs>
        </Stack>
      ...
      <Box sx={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative', zIndex: 10 }}>
        ...
        >
          <Box sx={{ borderBottom: `1px solid ${t.border}`, px: 2 }}>
            <Tabs
              value={showResults ? 1 : 0}
              onChange={(_, v) => setShowResults(v === 1)}
              sx={{ minHeight: 48, '& .MuiTab-root': { fontWeight: 800, fontSize: '0.75rem', minHeight: 48 } }}
            >
              <Tab label="Problem" icon={<Description sx={{ fontSize: 16 }} />} iconPosition="start" />
              <Tab
                label="Submissions"
                icon={<History sx={{ fontSize: 16 }} />}
                iconPosition="start"
                disabled={submitResults.length === 0 && !isSubmitting}
              />
            </Tabs>
          </Box>
          <Box sx={{ flex: 1, overflow: 'auto', p: 3 }}>
            <AnimatePresence mode="wait">
              {!showResults ? (
                <MotionBox key="desc" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                  <Typography variant="h2" sx={{ fontWeight: 900, fontSize: '1.8rem', mb: 2, letterSpacing: '-0.03em' }}>
                    {question.title}
                  </Typography>
                  <Stack direction="row" spacing={1} mb={4}>
                    <Chip
                      ...
                      size="small"
                      sx={{ fontWeight: 800, fontSize: '0.7rem', color: t.success, bgcolor: `${t.success}15`, borderRadius: '6px' }}
                    />
                    <Chip
                      label={topic}
                      size="small"
                      sx={{ fontWeight: 800, fontSize: '0.7rem', color: t.accent, bgcolor: `${t.accent}15`, borderRadius: '6px' }}
                    />
                  </Stack>
                  <Box sx={{ mb: 5 }}>
                    <Typography sx={{ fontWeight: 900, fontSize: '0.75rem', color: t.textMuted, mb: 2 }}>DESCRIPTION</Typography>
                    <Box
                      sx={{
                    ...
                    >
                      <div dangerouslySetInnerHTML={{ __html: question.description || question.problemStatement }} />
                    </Box>
                  </Box>
                  {question.example && (
                    <Box sx={{ mb: 5 }}>
                      <Typography sx={{ fontWeight: 900, fontSize: '0.75rem', color: t.textMuted, mb: 2 }}>EXAMPLE</Typography>
                      <Box sx={{ bgcolor: t.bg, p: 3, borderRadius: '20px', border: `1px solid ${t.border}` }}>
                        <Stack spacing={2}>
                          <Box>
                            <Typography variant="overline" sx={{ color: t.textMuted, fontWeight: 900 }}>
                              Input
                            </Typography>
                            <Typography
                              sx={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: t.accent, whiteSpace: 'pre-wrap' }}
                            >
                          ...
                          <Box>
                            ...
                            <Typography
                              sx={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: t.success, whiteSpace: 'pre-wrap' }}
                            >
                              {question.example.output?.split('\\n').join('\n')}
                            </Typography>
                          </Box>
                          {question.example.explanation && (
                            <Box sx={{ pt: 1, borderTop: `1px dashed ${t.border}` }}>
                              <Typography sx={{ fontSize: '0.9rem', color: t.textSecondary }}>{question.example.explanation}</Typography>
                            </Box>
                          )}
                        </Stack>
                      </Box>
                  ...
                  {question.constraints && (
                    <Box sx={{ mb: 5 }}>
                      <Typography sx={{ fontWeight: 900, fontSize: '0.75rem', color: t.textMuted, mb: 2 }}>CONSTRAINTS</Typography>
                      <Stack spacing={1}>
                        {(Array.isArray(question.constraints) ? question.constraints : [question.constraints]).map((c, i) => (
                          <Typography
                            key={i}
                            sx={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, fontSize: '0.85rem', color: '#f97316' }}
                          >
                            • {c}
                          </Typography>
                        ))}
                ...
                <MotionBox key="results" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}>
                  <Typography
                    variant="h3"
                    sx={{ fontWeight: 950, color: isSubmitting ? t.text : isAllPassed ? t.success : t.error, mb: 3 }}
                  >
                    {isSubmitting ? 'VERIFYING...' : isAllPassed ? 'ACCEPTED' : 'WRONG ANSWER'}
                  </Typography>
                  <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mb: 4 }}>
                    <Box sx={{ p: 2, bgcolor: t.bg, borderRadius: '16px', border: `1px solid ${t.border}`, textAlign: 'center' }}>
                      <Typography variant="overline" sx={{ color: t.textMuted, fontWeight: 800 }}>
                        Runtime
                      </Typography>
                      <Typography sx={{ fontWeight: 900 }}>{isSubmitting ? '...' : `${submitStats.runtime}s`}</Typography>
                    </Box>
                    <Box sx={{ p: 2, bgcolor: t.bg, borderRadius: '16px', border: `1px solid ${t.border}`, textAlign: 'center' }}>
                      <Typography variant="overline" sx={{ color: t.textMuted, fontWeight: 800 }}>
                        Memory
                      </Typography>
                      <Typography sx={{ fontWeight: 900 }}>{isSubmitting ? '...' : `${submitStats.memory}MB`}</Typography>
                    </Box>
                  </Box>
                  <Stack spacing={1.5}>
        ...
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', bgcolor: t.bg }} ref={editorSplitRef}>
          ...
          >
            <Stack direction="row" spacing={2}>
              ...
              <Stack direction="row" alignItems="center" spacing={1}>
                <IconButton size="small" onClick={() => setFontSize(fontSize - 2)}>
                  <Remove sx={{ fontSize: 14 }} />
                </IconButton>
                <Typography sx={{ fontWeight: 800, fontSize: '0.75rem' }}>{fontSize}</Typography>
                <IconButton size="small" onClick={() => setFontSize(fontSize + 2)}>
                  <Add sx={{ fontSize: 14 }} />
                </IconButton>
              </Stack>
            ...
            <Stack direction="row" spacing={1.5}>
              <Button
                variant="outlined"
                startIcon={isRunning ? <CircularProgress size={14} /> : <PlayArrow />}
                onClick={handleRun}
                disabled={isRunning}
                sx={{ height: 32, textTransform: 'none', fontWeight: 800 }}
              >
                Run
              </Button>
              <Button
                ...
                sx={{ height: 32, textTransform: 'none', fontWeight: 900, bgcolor: t.accent }}
              >
                Submit
              </Button>
            </Stack>
          ...
          <Box sx={{ height: `${compilerSplit}%` }}>
            <Editor
              ...
              options={{ fontSize, minimap: { enabled: false }, padding: { top: 20 }, fontFamily: "'JetBrains Mono', monospace" }}
            />
          </Box>

          <Box
            onMouseDown={startVerticalResizing}
            sx={{ height: 4, cursor: 'row-resize', bgcolor: t.border, '&:hover': { bgcolor: t.accent } }}
          />
          ...
          <Box sx={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            >
                    ...
                    >
                      ...
                      <Typography
                        sx={{
                          fontSize: '0.75rem',
                          fontWeight: 900,
                          color: statusColor !== 'transparent' ? statusColor : isActive ? t.accent : t.textMuted
                        }}
                      >
                        Case {i + 1}
                      </Typography>
                    ...
                    >
                      ...
                      <Stack direction="row" sx={{ ml: 1 }}>
                        <IconButton
                          ...
                          sx={{ p: 0.2, color: isActive ? t.accent : t.textMuted }}
                        >
                        ...
                        <IconButton
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDeleteCustom(ci);
                          }}
                          sx={{ p: 0.2, color: t.error }}
                        >
              ...
              <Button
                ...
                disabled={customTestCases.length >= 4}
                size="small"
                startIcon={<Add />}
                sx={{ ml: 1, height: 32, fontSize: '0.7rem', fontWeight: 900, bgcolor: 'transparent', color: t.accent }}
              >
                Add Case ({customTestCases.length}/4)
              </Button>
            </Box>
            ...
            <Box sx={{ flex: 1, p: 3, overflow: 'auto' }}>
              {(() => {
                ...
                if (!activeTC)
                  return (
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: t.textMuted }}>
                      <Typography sx={{ fontWeight: 700 }}>Choose a case to analyze</Typography>
                    </Box>
                  );
                ...
                return (
                  <Stack spacing={3}>
                    <Box>
                      <Typography variant="overline" sx={{ fontWeight: 900, color: t.textMuted, fontSize: '0.65rem' }}>
                        INPUT
                      </Typography>
                      <Box
                        sx={{
                      ...
                      >
                        {(activeTC.input?.value || activeTC.input)?.split('\\n').join('\n')}
                      </Box>
                    </Box>
                    {activeRes && (
                      <Box>
                        <Typography
                          variant="overline"
                          sx={{ fontWeight: 900, color: activeRes.passed ? t.success : t.error, fontSize: '0.65rem' }}
                        >
                          YOUR OUTPUT
                        </Typography>
                        <Box
                          sx={{
                            p: 2,
                            bgcolor: activeRes.passed ? `${t.success}05` : `${t.error}05`,
                            border: `1px solid ${activeRes.passed ? t.success : t.error}30`,
                            borderRadius: '12px',
                            fontFamily: "'JetBrains Mono', monospace",
                            fontSize: '0.85rem',
                            whiteSpace: 'pre-wrap'
                        ...
                        >
                          {activeRes.output || activeRes.error}
                        </Box>
                      </Box>
                    )}
                    {currentTestCaseTab < publicTC.length && (
                      <Box>
                        ...
                        >
                          {(activeTC.output?.value || activeTC.output)?.split('\\n').join('\n')}
                        </Box>
                      </Box>
                    )}
                  </Stack>
      ...
      >
        <DialogTitle sx={{ fontWeight: 900, fontSize: '1.4rem' }}>
          {editIndex !== null ? 'Modify Custom Case' : 'New Custom Case'}
        </DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ color: t.textMuted, mb: 2, fontWeight: 700 }}>
            Type your input exactly as expected by the program.
          </Typography>
          <TextField
            fullWidth
            multiline
            ...
            sx={{
              mt: 1,
              '& .MuiOutlinedInput-root': { borderRadius: '16px', bgcolor: t.bgSide, fontFamily: "'JetBrains Mono', monospace", p: 2 }
            }}
          />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setShowAddCustom(false)} sx={{ fontWeight: 700, color: t.textMuted }}>
            Cancel
          </Button>
          <Button variant="contained" onClick={handleSaveCustom} sx={{ bgcolor: t.accent, borderRadius: '12px', fontWeight: 900 }}>
