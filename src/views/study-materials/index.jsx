...
export default function StudyMaterials() {
  ...
  const filteredMaterials = materials.filter(
    (m) => m.title.toLowerCase().includes(searchQuery.toLowerCase()) || m.description?.toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        >
          {[
            { label: 'Total Courses', value: materials.length, icon: <IconBook size={18} /> },
            {
              ...
              value: materials.reduce((acc, m) => acc + (m.chapters?.length || 0), 0),
              icon: <IconBookmark size={18} />
            },
            {
              label: 'Total Lessons',
              value: materials.reduce((acc, m) => acc + (m.chapters?.reduce((a, ch) => a + (ch.lessons?.length || 0), 0) || 0), 0),
